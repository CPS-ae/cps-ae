...
RAND_FILE="CPU_Load_RAND""__$NOW.log"
STARTUP_TIME=2
...
then
   echo "[$(get_current_time)] => Starting stress for $STARTUP_TIME seconds."\
    ...
    nice -n -5 ./stress --cpu $NUMBER_OF_CORES --timeout $CPU_LOAD_DURATION --quiet &
#    nice -n -5 stress --cpu $NUMBER_OF_CORES --timeout $CPU_LOAD_DURATION --quiet &
#    nice -n -5 stress --cpu $NUMBER_OF_CORES --timeout $CPU_LOAD_DURATION --quiet &
#    nice -n -5 stress --cpu $NUMBER_OF_CORES --timeout $CPU_LOAD_DURATION --quiet &
   ...
   array=(${STRESS_PIDS// / })
   #readarray -t rndArr < bestbest.log
   for PID in "${array[@]}"
   #for (( i=0; i<${#array[*]}; ++i ))
   do
      #PID=${array[$i]}
      #CURRENT_RAND=${rndArr[$i]}
      #echo $PID $CURRENT_RAND
      #Send standard output and error messages to the null device (bit bucket)
       taskset -pc $CURRENT_CORE_NUMBER $PID >/dev/null 2>&1
      ##if [ $(($CURRENT_CORE_NUMBER % 2)) -eq 0 ]
      ##then
      ##    CURRENT_RAND=$(($RANDOM % 20 + 20))
      ##else
      ##    CURRENT_RAND=$(($RANDOM % 20 + 80))
      ##fi
      CURRENT_RAND=$(($RANDOM % 40 + 60))
      #CURRENT_RAND=$(($RANDOM % 40))
      echo $CURRENT_RAND >> $RAND_FILE
      #echo $CURRENT_RAND >> $RAND_FILE
       ./cpulimit -p $PID -l $CURRENT_RAND &
      # cpulimit -p $PID -l 100 -b >/dev/null 2>&1
